...
class LandoltTopUIView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        topRect.move(to: CGPoint(x: self.frame.width*0.4, y: 0))
        topRect.addLine(to: CGPoint(x: self.frame.width*0.6, y: 0))
        topRect.addLine(to: CGPoint(x: self.frame.width*0.6, y: self.frame.height/2))
        topRect.addLine(to: CGPoint(x: self.frame.width*0.4, y: self.frame.height/2))
        topRect.close()
        UIColor.white.setFill()
        topRect.fill()
